		...
		if  len(item["theid"].(primitive.A)) > 0 && item["owner"].(string)  == id {
			...
			if item["opened"] == nil {
				...
				if item["type"].(string) == "others branch" || item["type"].(string) == "poll" {
					details = append(details, "<a href='/?box=ding&id="+item["postid"].(string)+"&target=notification_check&notify="+item["_id"].(primitive.ObjectID).Hex()+"#cmmshw"+item["postid"].(string)+"' style='text-decoration:none;color:black;'>")
				}
				
	
				if item["type"].(string) == "like" || item["type"].(string) == "tagger" || item["type"].(string) == "tag" ||  item["type"].(string) == "share" {
					details = append(details, "<a href='/?box=ding&id="+item["postid"].(string)+"&target=notification_check&notify="+item["_id"].(primitive.ObjectID).Hex()+"' style='text-decoration:none;color:black;'>")
				}
	
				if item["type"].(string) == "subscribe" {
					details = append(details, "<a href='/"+item["username"].(string)+"&target=notification_check&notify="+item["_id"].(primitive.ObjectID).Hex()+"' style='text-decoration:none;color:black;'>")
				} 
