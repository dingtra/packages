...
func (let *DingtagStruct ) TagNameCommentReply( r *http.Request) {
	...
	if strings.ToLower(r.Method) == "post" {
		...
		cmmntid := strings.ToLower(strings.TrimSpace(r.FormValue("cmmntid")))
		getnames := []string{}
		fmt.Println(name, " III")
		...
		for _, item := range usernames {
			...
			if matched == true || getfullname == true {
				...
				if let.Usersid  != item["_id"].(primitive.ObjectID).Hex(){

					//open
					getnames = append(getnames, "<div id='tgreplys' class='usrtg6790' mx='"+item["_id"].(primitive.ObjectID).Hex()+"' gx='"+item["username"].(string)+"' x='"+cmmntid+"'><div style='pointer-events:none;' class='usrtg6790-ch'>")
		...
		if len(getnames) > 0 {
			...
			Error["nwdngxr092"] = template.HTML("<div class='sug009'>"+strings.Join(getnames, "")+"</div>")
		}else{
			Error["nwdngxr092"]  = template.HTML("<div class='sug009'><div style='padding:10px 5px 10px 5px;'><span style='font-size:14px;color:red;'>No record for <b>"+name+"</b></span></div></div>")
		}
