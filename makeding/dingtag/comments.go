...
func (let *DingtagStruct ) TagNameComments( r *http.Request) {
		...
		for _, item := range usernames {
			...
			if matched == true || getfullname == true {
				...
				if let.Usersid  != item["_id"].(primitive.ObjectID).Hex(){
					...
					//open
					getnames = append(getnames, "<div id='tgcmmnts' class='usrtg6790' mx='"+item["_id"].(primitive.ObjectID).Hex()+"' gx='"+item["username"].(string)+"' x='"+cmmntid+"'><div style='pointer-events:none;' class='usrtg6790-ch'>")
