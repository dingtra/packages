...
func (let *DingtagStruct ) TagName( r *http.Request) {
	getcoll := rundb.Connect("app").Collection("register")
	usernames := rundb.FindAll(getcoll)
		...
		for _, item := range usernames {
			...
			if matched == true || getfullname == true {
				...
				if let.Usersid  != item["_id"].(primitive.ObjectID).Hex(){
					...
					//open
					getnames = append(getnames, "<div id='tgall091' class='usrtg6790' mx='"+item["_id"].(primitive.ObjectID).Hex()+"' gx='"+item["username"].(string)+"'><div style='pointer-events:none;' class='usrtg6790-ch'>")
